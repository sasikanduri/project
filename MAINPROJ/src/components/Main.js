 ...
 */
 ...
 const localTheme = getMuiTheme(Object.assign({}, {
	palette: {
...
class Main extends Component {
	...
	handleUpdateTask = (task) => {
		this.setState(previousState => {
			...
			task.status = (task.status === 'Write_It') ? 'Completed' : 'Write_It';
	...
	render() {
		...
		const columns = [
			{ title: 'Write_It', items: items.filter( item => item.status === 'Write_It'), icon: <TodoIcon />},
			{ title: 'Completed', items: items.filter( item => item.status === 'Completed'), icon: <CheckIcon />},
			{ title: 'All Tasks', items, icon: <ListIcon />},
		];
		return (
			<MuiThemeProvider muiTheme={localTheme}>
				<div className="App">
					...
					/>
					<AppBar
						title={<span style={{color: 'azure',fontWeight:'bolder'}}>WriteIt_DoIt</span>}
						showMenuIconButton={false}
						style={{backgroundColor: 'navy', position: 'fixed', zIndex: 9999,}}
					/>
					<div className="App-container">
						<div style={{position: 'fixed', width: '100%', paddingTop:40, zIndex: 8888, backgroundColor: 'rgb(216, 199, 222)'}}>
							<TextField
								hintText="Enter the Task"
								floatingLabelText="Add Tasks"
								ref={(taskInput) => {
									this.taskInput = taskInput;
								}}
								disabled={this.state.removeMode}
								style={{margin: 10, width: '80%', maxWidth: 300}}
							...
							>
								{columns.map((column,index) => (
									<Tab
										...
										label={
											<div>
												{column.title} ({(column.title !== 'All Tasks') ? column.items.filter(item => item.status === column.title).length: items.length})
											</div>
										}
									/>
								))}
